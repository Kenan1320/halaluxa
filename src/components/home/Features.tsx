...
const features = [
  {
    icon: <ShoppingBag className="w-10 h-10 text-haluna-primary" />,
  ...
  {
    icon: <Users className="w-10 h-10 text-haluna-primary" />,
  ...
  {
    icon: <CreditCard className="w-10 h-10 text-haluna-primary" />,
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300 animate-slide-in-bottom"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="bg-haluna-primary-light rounded-full w-16 h-16 flex items-center justify-center mb-5 mx-auto">
                {feature.icon}
              </div>
              <h3 className="text-xl font-medium text-haluna-text mb-3 text-center">{feature.title}</h3>
              <p className="text-haluna-text-light text-center">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
